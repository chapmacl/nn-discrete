...
def train_model(real_model_folder):
    ...
    discrete_net.evaluate_and_save_to_disk(test_loader, "FashionMnist-ex3.1_untrained_discretized_ternary_sample")
    discrete_net = logit_net.generate_discrete_networks("argmax")
    discrete_net = discrete_net.to(device)
    discrete_net.evaluate_and_save_to_disk(test_loader, "FashionMnist-ex3.1_untrained_discretized_ternary_argmax")

    # evaluate first logit model before training, train and evaluate again
    logit_net.train_model(train_loader, validation_loader, test_loader, 100, "FashionMnistTernary", True)

    # discretizing trained logits net and evaluating
    discrete_net = logit_net.generate_discrete_networks("sample")
    discrete_net = discrete_net.to(device)
    discrete_net.evaluate_and_save_to_disk(test_loader, "FashionMnist-ex4.1_trained_discretized_ternary_sample")
    discrete_net = logit_net.generate_discrete_networks("argmax")
    discrete_net = discrete_net.to(device)
    discrete_net.evaluate_and_save_to_disk(test_loader, "FashionMnist-ex4.1_trained_discretized_ternary_argmax")
