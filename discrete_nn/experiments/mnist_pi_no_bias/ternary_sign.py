...
from discrete_nn.models.pi_no_bias.logit_sign import PiNoBiasLogitSign
...
def train_model(real_model_folder):
    ...
    logit_net = PiNoBiasLogitSign(real_param, DiscreteWeights.TERNARY)
    logit_net = logit_net.to(device)

    # evaluate first logit model before training, train and evaluate again

    logit_net.train_model(train_loader, validation_loader, test_loader, 200, "MNIST-Pi-NoBias-Sign-Ternary", True)
