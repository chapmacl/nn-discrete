...
class LogitBatchNorm_INCOMPLETE(Module):
    def __init__(self, input_format: InputFormat, num_features, initialization_weights, initialization_bias,
        ...
        :param discrete_weights: the discrete weights to use
        of channels
        """
        super().__init__()
        self._input_format = input_format
        self._num_feat = num_features
        self.weight = torch.nn.Parameter(discretize_weights_probabilistic(initialization_weights,
                                                                          self.discrete_values),
                                         requires_grad=True)
        self.bias = torch.nn.Parameter(discretize_weights_probabilistic(initialization_bias, self.discrete_values),
                                       requires_grad=True)
        self.discrete_values = sorted(discrete_weights.value)
    ...
    def forward(self, x):
        ...
        weights_mean, weights_std = get_gaussian_dist_parameters(self.weight, self.discrete_values)
        bias_mean, bias_std = get_gaussian_dist_parameters(self.bias, self.discrete_values)
