...
class LayerConv(LayerLinearForward):
    def __init__(self,
        ...
        self._fan_in = float(
            self._n_features_in * self._kernel_size[0] * self._kernel_size[1] + (1 if self._enable_bias else 0))
        ...
        if self._enable_bias:
            ...
            q_distributions[bias_type]((self._W_shape[0],), bias_parameterization, bias_initialization_method,
        ...
        if self._tf_same_padding == True:
            ...
            if self._input_layer.isOutputDistribution():
                ...
                if self._tf_same_padding_col:
                    zero_pad = T.zeros(
                        (x_mean.shape[0], in_shape[0], in_shape[1] + (1 if self._tf_same_padding_row else 0), 1),
                ...
                if self._tf_same_padding_col:
                    zero_pad = T.zeros(
                        (x.shape[0], in_shape[0], in_shape[1] + (1 if self._tf_same_padding_row else 0), 1),
                        theano.config.floatX)
    ...
    def getOutputShape(self):
        ...
        if self._border_mode == 'valid':
            ...
            if self._border_mode == 'full':
                ...
                raise Exception(
                    'Unknown border mode \'%s\'. We should not be here (error should have been catched earlier).' % (
                        self._border_mode))
    ...
    def getMessage(self):
        ...
        return self._input_layer.getMessage() + '\n%20s: OutputShape=%15s, DistributionOutput=%d, Parameters=%s, WeightType=%s, Parameterization=%s, InitMethod=%s, KernelSize=%s, Stride=%s, RegularizerType=%s, RegularizerWeight=%s, RegularizerParameter=%s' % (
        'LayerConv', str(self.getOutputShape()), self.isOutputDistribution(), str(param_names), self._weight_type,
        self._weight_parameterization, self._weight_initialization_method, str(self._kernel_size), str(self._stride),
        str(self._regularizer), str(self._regularizer_weight), str(self._regularizer_parameters))
