...
class LayerLinearForward(Layer):
    ...
    def getSymbolicWeights(self, weight_types):
        ...
        for weight_type in weight_types:
            if weight_type == 'raw':
                ...
                [t[1] for t in self._parameters_names if t[0] == 'scale_factor_rho'][0],)
            else:
                raise NotImplementedError()

        return self._input_layer.getSymbolicWeights(weight_types) + [symbolic_weights]

    def _addRealWeights(self, shape, parameterization, initialization_method, initial_parameters, regularizer,
    ...
    def _addGaussDistribution(self, shape, parameterization, initialization_method, initial_parameters, regularizer,
                              regularizer_weight, regularizer_parameters, enable_scale_factors, rng, srng):
        assert enable_scale_factors == False
        ...
        if initial_parameters is None:
            ...
            if param_name_mu in initial_parameters and param_name in initial_parameters:
                raise Exception(
                    'Initial parameters contain \'%s\' and \'%s\'. Do not know which initial parameters to use.' % (
                    param_name_mu, param_name))
            ...
            if param_name_sigma_rho in initial_parameters:
                W_sigma_rho_values = LayerLinearForward._assignParameters(initial_parameters, param_name_sigma_rho,
                                                                          shape)
            else:
                print 'Note: Initialization parameter \'%s\' not found. Using default initial values for the variance.' % (
                    param_name_sigma_rho)
                W_sigma_rho_values = np.full(shape, -5., dtype=theano.config.floatX)
        ...
        if regularizer == 'kl':
            ...
            cost_prior = 0.5 * (log_2_pi_gamma * np.prod(shape) + (1. / regularizer_parameters) * (
                        T.sum(T.sqr(W_mu)) + T.sum(W_var)))
    ...
    def _addTernaryDistribution(self, shape, parameterization, initialization_method, initial_parameters,
        ...
        if initial_parameters is None or param_name in initial_parameters:
            if initial_parameters is not None:
                ...
                if not (enable_scale_factors and initialization_method_scale_factors == 'kmeans'):
                    ...
                    W_values[W_values <= 0.] = self._mapToEcdf(
                        W_values[W_values <= 0.]) * 1.5 - 1.5  # Use the empirical cdf to initialize
            ...
            if enable_scale_factors == True:
                if initialization_method_scale_factors == 'default':
                    ...
                    w_discrete_negative, _ = self._kmeansPositiveWeightsFixedZero(-W_values[W_values <= 0], k=1,
                                                                                  n_restarts=1, init='random_samples',
                                                                                  rng=rng, n_iter=500)
                    w_discrete_positive, _ = self._kmeansPositiveWeightsFixedZero(W_values[W_values > 0], k=1,
                                                                                  n_restarts=1, init='random_samples',
                                                                                  rng=rng, n_iter=500)
                    w_discrete_values = np.asarray([-w_discrete_negative[1], 0., w_discrete_positive[1]],
                    ...
                    assert False
                scale_factor_rho_values = np.asarray([-w_discrete_values[0], w_discrete_values[2]],
            ...
            if initialization_method in ['shayer', 'expectation']:
                ...
                pB_values = (W_values / (1. - pA_values) - w_discrete_values[0]) / (
                            -w_discrete_values[0] + w_discrete_values[2])
            ...
            if parameterization == 'shayer':
                ...
                W_rho_values = np.log(np.stack([p_m1_values, p_zr_values, p_p1_values], axis=-1)).astype(
                    theano.config.floatX)
            elif parameterization == 'logits_fixedzero':
            ...
            if enable_scale_factors:
                if param_name_scale_factor in initial_parameters:
                    scale_factor_rho_values = LayerLinearForward._assignParameters(initial_parameters,
                                                                                   param_name_scale_factor, (2,))
                else:
        ...
        if enable_scale_factors:
            ...
            W_var = W_p_m1 * T.sqr(scale_factor[0] + W_mean) + W_p_0 * T.sqr(W_mean) + W_p_p1 * T.sqr(
                scale_factor[1] - W_mean)
        ...
        if regularizer == 'shayer':
            ...
            cost_prior = -(T.log(0.5 * (1. - regularizer_parameters)) * np.prod(shape) + T.log(
                2. * regularizer_parameters / (1. - regularizer_parameters)) * T.sum(W_p[..., 1]))
            cost_regularizer = (cost_entropy + cost_prior) * regularizer_weight
    ...
    #
    #################################################################################
    def _addQuaternaryDistribution(self, shape, quaternary_type, parameterization, initialization_method,
        ...
        if initial_parameters is None or param_name in initial_parameters:
                ...
                if not (enable_scale_factors and initialization_method_scale_factors == 'kmeans'):
                    ...
                    if quaternary_type == 'symmetric':
                        W_values[W_values <= 0.] = self._mapToEcdf(W_values[W_values <= 0.]) * (4. / 3.) - (
                                    4. / 3.)  # Use the empirical cdf to initialize
                        W_values[W_values > 0.] = self._mapToEcdf(W_values[W_values > 0.]) * (4. / 3.)
                    elif quaternary_type == 'fixed_point_minus':
                        W_values[W_values <= 0.] = self._mapToEcdf(
                            W_values[W_values <= 0.]) * 0.75 - 0.75  # Use the empirical cdf to initialize
            ...
            if parameterization == 'logits':
                print 'Debug init QUATERNARY weights: errs:', np.sum(np.abs(
                    np.sum(np.stack([p_A_values, p_B_values, p_C_values, p_D_values], axis=-1), axis=-1) - 1) >= 1e-6)
                W_rho_values = np.log(np.stack([p_A_values, p_B_values, p_C_values, p_D_values], axis=-1)).astype(
                    theano.config.floatX)
        ...
        if enable_scale_factors:
            ...
            W_epsilon = srng.uniform(shape + (1,), dtype=theano.config.floatX)
            if quaternary_type == 'symmetric':
                W_resampled = (T.sum(T.cast(T.le(W_sample_cumsum, W_epsilon), theano.config.floatX), axis=-1) - 1.5) * (
                            2. / 3.)
            elif quaternary_type == 'fixed_point_minus':
                W_resampled = (T.sum(T.cast(T.le(W_sample_cumsum, W_epsilon), theano.config.floatX),
                                     axis=-1) - 2.0) * 0.5
            elif quaternary_type == 'fixed_point_plus':
                W_resampled = (T.sum(T.cast(T.le(W_sample_cumsum, W_epsilon), theano.config.floatX),
                                     axis=-1) - 1.0) * 0.5
    ...
    #
    #################################################################################
    def _addQuinaryDistribution(self, shape, parameterization, initialization_method, initial_parameters, regularizer,
                                regularizer_weight, regularizer_parameters, enable_scale_factors, rng, srng):
        if parameterization is None:
            parameterization = 'shayer'
        if initialization_method is None:
        ...
        if initial_parameters is None or param_name in initial_parameters:
            if initial_parameters is not None:
                ...
                if not (enable_scale_factors and initialization_method_scale_factors == 'kmeans'):
                    ...
                    W_values[W_values <= 0.] = self._mapToEcdf(
                        W_values[W_values <= 0.]) * 1.25 - 1.25  # Use the empirical cdf to initialize
            ...
            if enable_scale_factors == True:
                if initialization_method_scale_factors == 'default':
                    ...
                    # TODO: Perform a few random restarts and pick the best result
                    w_discrete_negative, _ = self._kmeansPositiveWeightsFixedZero(-W_values[W_values <= 0], k=2,
                                                                                  n_restarts=1, init='random_samples',
                                                                                  rng=rng, n_iter=500)
                    w_discrete_positive, _ = self._kmeansPositiveWeightsFixedZero(W_values[W_values > 0], k=2,
                                                                                  n_restarts=1, init='random_samples',
                                                                                  rng=rng, n_iter=500)
                    w_discrete_values = np.asarray(
                        [-w_discrete_negative[2], -w_discrete_negative[1], 0., w_discrete_positive[1],
                         w_discrete_positive[2]], dtype=theano.config.floatX)
                else:
                    assert False
                scale_factor_rho_values = np.asarray([w_discrete_values[1] - w_discrete_values[0],
                                                      -w_discrete_values[1],
                                                      w_discrete_values[3],
                                                      w_discrete_values[4] - w_discrete_values[3]],
                                                     theano.config.floatX)
                scale_factor_rho_values = np.log(np.exp(scale_factor_rho_values) - 1.)
            ...
            if initialization_method in ['shayer', 'expectation']:
                ...
                pC_values_aux = ((W_values / (1. - pA_values) - 0.5 * (1. - pB_values) * (
                            w_discrete_values[0] + w_discrete_values[1])) / pB_values - w_discrete_values[3]) / (
                                            w_discrete_values[4] - w_discrete_values[3])
                ...
                pD_values_aux = ((W_values / (1. - pA_values) - 0.5 * pB_values * (
                            w_discrete_values[4] + w_discrete_values[3])) / (1. - pB_values) - w_discrete_values[1]) / (
                                            w_discrete_values[0] - w_discrete_values[1])
            ...
            if parameterization == 'shayer':
                ...
                W_rho_values = np.log(
                    np.stack([p_m1_values, p_m05_values, p_zr_values, p_p05_values, p_p1_values], axis=-1)).astype(
                    theano.config.floatX)
            elif parameterization == 'logits_fixedzero':
        ...
        if parameterization == 'shayer':
            ...
            W_p = T.stack([W_p_m1, W_p_m05, W_p_0, W_p_p05, W_p_p1],
                          axis=-1)  # These probabilities are already normalized
        elif parameterization == 'logits':
            W_rho = theano.shared(W_rho_values, borrow=True)
            self._addParameterEntry(W_rho, param_name_rho, is_trainable=True, bounds=self._logit_bounds)
            W_p = W_rho - T.max(W_rho, axis=-1, keepdims=True)
            ...
            self._addParameterEntry(W_rho_p1, param_name_rho_p1, is_trainable=True, bounds=self._logit_bounds)
            W_rho_full = T.stack([W_rho_m1, W_rho_m05, T.zeros(shape, theano.config.floatX), W_rho_p05, W_rho_p1],
                                 axis=-1)
            W_p = W_rho_full - T.max(W_rho_full, axis=-1, keepdims=True)
            W_p = T.exp(W_p)
            W_p = W_p / T.sum(W_p, axis=-1, keepdims=True)
    ...
    def _kmeansPositiveWeightsFixedZero(weights, k, n_restarts=1, init='linear', rng=None, n_iter=100):
        ...
        for _ in range(n_restarts):
            centers, cumdist = LayerLinearForward._kmeansPositiveWeightsFixedZeroImpl(weights, k, init='linear',
                                                                                      rng=None, n_iter=100)
